...
export type ExitSignal = "HOLD" | "TRIM" | "EXIT";
export type InvestorStyle = "value" | "growth" | "core";

export interface Factor { name: string; score: number; explanation: string; source: string; }
export interface DemoScore { score: number; verdict: Verdict; summary: string; factors: Factor[]; }
export interface DemoQuote {
  companyName: string; price: number; change: number; changePercent: number;
  marketCap: number; beta: number; sector: string; industry: string; country: string;
  exchangeShortName: string; "52wHigh": number; "52wLow": number;
  sharesOutstanding: number; description: string;
}
export interface FinancialYear {
  date: string; revenue: number; grossProfit: number; operatingIncome: number;
  netIncome: number; operatingIncomeRatio: number; totalAssets: number;
  totalLiabilities: number; totalEquity: number; debtToEquity: number;
  operatingCashFlow: number; investingCashFlow: number; financingCashFlow: number;
  freeCashFlow: number; capitalExpenditure: number;
}
export interface DemoRatios {
  peRatio: number; forwardPE: number; priceToBook: number; priceToSales: number;
  fcfYield: number; grossMargin: number; operatingMargin: number; netMargin: number;
  revenueGrowth: number; dividendYield: number; returnOnEquity: number;
  returnOnAssets: number; debtToEquity: number; currentRatio: number;
}

export const DEMO_TICKERS = ["AAPL","NVDA","MSFT","TSLA","GOOGL","AMZN"] as const;
...
// ── Profile-adjusted scores ───────────────────────────────────────────────────
// Weights: value → valuation+15%, growth → fundamentals+15%, core → balanced
const WEIGHTS: Record<InvestorStyle, Record<string, number>> = {
  value:  { Fundamentals: 0.20, Moat: 0.20, Risk: 0.15, Valuation: 0.35, Management: 0.10 },
  growth: { Fundamentals: 0.35, Moat: 0.25, Risk: 0.15, Valuation: 0.15, Management: 0.10 },
  core:   { Fundamentals: 0.25, Moat: 0.25, Risk: 0.20, Valuation: 0.20, Management: 0.10 },
};

export function profileScore(ticker: DemoTicker, style: InvestorStyle): number {
  const s = DEMO_SCORES[ticker];
  const w = WEIGHTS[style];
  const weighted = s.factors.reduce((acc, f) => acc + f.score * (w[f.name] ?? 0.2), 0);
  return Math.round(Math.min(weighted, 100));
}

export function profileSummary(ticker: DemoTicker, style: InvestorStyle): string {
  const base = profileScore(ticker, style);
  const defaultScore = DEMO_SCORES[ticker].score;
  const diff = base - defaultScore;
  const label = style === "value" ? "value investor" : style === "growth" ? "growth investor" : "balanced";
  if (Math.abs(diff) < 3) return `Score is nearly identical to the default for a ${label} profile.`;
  if (diff > 0) return `As a ${label}, this stock scores ${diff} points higher — ${style === "growth" ? "strong fundamentals favour this profile" : style === "value" ? "valuation is more attractive than the default implies" : "well-balanced across all dimensions"}.`;
  return `As a ${label}, this stock scores ${Math.abs(diff)} points lower — ${style === "value" ? "valuation is a concern at current levels" : style === "growth" ? "fundamentals weigh down this profile" : "risk factors reduce the score"}.`;
}
...
export interface ExitAnalysis {
  exitScore: number;
  signal: ExitSignal;
  drivers: { label: string; detail: string; source: string }[];
  framing: string;
}

// Deterministic exit analysis based on current score vs typical entry score
export function computeExitAnalysis(ticker: DemoTicker, purchasePrice?: number): ExitAnalysis {
  const q = DEMO_QUOTES[ticker];
  const s = DEMO_SCORES[ticker];
  const priceReturn = purchasePrice ? ((q.price - purchasePrice) / purchasePrice) * 100 : null;

  // Exit score: lower = more reason to exit. Based on valuation factor + overall score
  const valuationFactor = s.factors.find(f => f.name === "Valuation")!;
  const riskFactor = s.factors.find(f => f.name === "Risk")!;

  // Deterministic formula: if valuation < 45 and overall < 55 → EXIT
  // if valuation < 55 or price return > 40% → TRIM
  // else HOLD
  const exitScore = Math.round((valuationFactor.score * 0.4 + s.score * 0.4 + riskFactor.score * 0.2));
  const hasHighReturn = priceReturn !== null && priceReturn > 30;
  const signal: ExitSignal = exitScore < 40 ? "EXIT" : (exitScore < 55 || hasHighReturn) ? "TRIM" : "HOLD";

  const drivers: { label: string; detail: string; source: string }[] = [
    { label: "Overall score", detail: `${s.score}/100 — ${s.score >= 65 ? "thesis intact" : s.score >= 50 ? "thesis weakening" : "thesis broken"}`, source: "pondex_ score" },
    { label: "Valuation signal", detail: `${valuationFactor.score}/100 — ${valuationFactor.explanation}`, source: "Yahoo Finance" },
    { label: "Risk level", detail: `${riskFactor.score}/100 — ${riskFactor.explanation}`, source: "Yahoo Finance" },
  ];
  if (priceReturn !== null) {
    drivers.push({ label: "Price return since entry", detail: `${priceReturn >= 0 ? "+" : ""}${priceReturn.toFixed(1)}% — ${Math.abs(priceReturn) > 30 ? "significant gain, consider trimming" : "within normal range"}`, source: "Entry price" });
  }

  const framing =
    signal === "EXIT" ? "Score decay and valuation signal suggest thesis conditions have changed — review recommended."
    : signal === "TRIM" ? "Consider reducing position size — score is weakening or gains are substantial."
    : "Thesis conditions remain intact — no exit signal at this time.";

  return { exitScore, signal, drivers, framing };
}
...
export const DEMO_QUOTES: Record<DemoTicker, DemoQuote> = {
  ...
  GOOGL: { companyName: "Alphabet Inc.", price: 198.12, change: 1.08, changePercent: 0.55, marketCap: 2410000000000, beta: 1.03, sector: "Communication Services", industry: "Internet Content & Information", country: "US", exchangeShortName: "NASDAQ", "52wHigh": 208.70, "52wLow": 155.63, sharesOutstanding: 12170000000, description: "Alphabet Inc. provides various products and platforms in the United States and internationally." },
...
// ── Ratios ────────────────────────────────────────────────────────────────────
export const DEMO_RATIOS: Record<DemoTicker, DemoRatios> = {
  AAPL:  { peRatio: 32.4, forwardPE: 28.1, priceToBook: 48.2, priceToSales: 8.1,  fcfYield: 0.031, grossMargin: 0.461, operatingMargin: 0.311, netMargin: 0.261, revenueGrowth: 0.042, dividendYield: 0.005, returnOnEquity: 1.601, returnOnAssets: 0.281, debtToEquity: 1.81, currentRatio: 1.04 },
  NVDA:  { peRatio: 54.2, forwardPE: 38.8, priceToBook: 38.1, priceToSales: 25.4, fcfYield: 0.018, grossMargin: 0.752, operatingMargin: 0.618, netMargin: 0.552, revenueGrowth: 0.942, dividendYield: 0.0003,returnOnEquity: 1.242, returnOnAssets: 0.582, debtToEquity: 0.41, currentRatio: 4.17 },
  MSFT:  { peRatio: 38.1, forwardPE: 31.4, priceToBook: 13.4, priceToSales: 14.2, fcfYield: 0.026, grossMargin: 0.698, operatingMargin: 0.442, netMargin: 0.362, revenueGrowth: 0.151, dividendYield: 0.0068,returnOnEquity: 0.362, returnOnAssets: 0.162, debtToEquity: 0.71, currentRatio: 1.28 },
  TSLA:  { peRatio: 88.4, forwardPE: 72.1, priceToBook: 14.8, priceToSales: 8.2,  fcfYield: 0.008, grossMargin: 0.178, operatingMargin: 0.062, netMargin: 0.058, revenueGrowth: 0.021, dividendYield: 0,     returnOnEquity: 0.118, returnOnAssets: 0.042, debtToEquity: 0.11, currentRatio: 1.84 },
  GOOGL: { peRatio: 24.8, forwardPE: 20.4, priceToBook: 7.2,  priceToSales: 6.8,  fcfYield: 0.038, grossMargin: 0.582, operatingMargin: 0.322, netMargin: 0.282, revenueGrowth: 0.122, dividendYield: 0,     returnOnEquity: 0.314, returnOnAssets: 0.162, debtToEquity: 0.08, currentRatio: 1.94 },
  AMZN:  { peRatio: 42.1, forwardPE: 34.8, priceToBook: 9.4,  priceToSales: 3.8,  fcfYield: 0.022, grossMargin: 0.482, operatingMargin: 0.108, netMargin: 0.088, revenueGrowth: 0.111, dividendYield: 0,     returnOnEquity: 0.224, returnOnAssets: 0.072, debtToEquity: 0.48, currentRatio: 1.06 },
};

// ── Financials ────────────────────────────────────────────────────────────────
export const DEMO_FINANCIALS: Record<DemoTicker, FinancialYear[]> = {
  AAPL: [
...
export const PEER_MAP: Record<DemoTicker, DemoTicker[]> = {
  AAPL: ["MSFT", "GOOGL", "AMZN"], NVDA: ["MSFT", "TSLA", "AMZN"],
  MSFT: ["AAPL", "GOOGL", "AMZN"], TSLA: ["AMZN", "GOOGL", "NVDA"],
  GOOGL: ["MSFT", "AAPL", "AMZN"], AMZN: ["MSFT", "GOOGL", "AAPL"],
};
...
export function generateCandles(startPrice: number, drift: number, volatility: number, days = 252): Candle[] {
...
export const DEMO_CANDLES: Record<DemoTicker, Candle[]> = {
  AAPL: generateCandles(164, 0.3, 0.018), NVDA: generateCandles(87, 0.82, 0.032),
  MSFT: generateCandles(386, 0.22, 0.015), TSLA: generateCandles(214, 0.48, 0.042),
  GOOGL: generateCandles(156, 0.27, 0.017), AMZN: generateCandles(167, 0.33, 0.019),
};

// ── Helpers ───────────────────────────────────────────────────────────────────
export function isDemoTicker(t: string): t is DemoTicker { return (DEMO_TICKERS as readonly string[]).includes(t.toUpperCase()); }
export function verdictFromScore(score: number): Verdict { return score >= 70 ? "BUY" : score >= 50 ? "HOLD" : "SELL"; }
export function verdictColor(score: number): "buy" | "hold" | "sell" { return score >= 70 ? "buy" : score >= 50 ? "hold" : "sell"; }
